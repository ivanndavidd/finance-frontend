...
export const api = {
  ...
  getCategoryStats: async (month?: string): Promise<CategoryStats[]> => {
    ...
    const response = await fetch(
      `${API_BASE_URL}/transactions/stats?${params}`
    );
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
  ...
  getDailyStats: async (month?: string): Promise<DailyStats[]> => {
    ...
    const response = await fetch(
      `${API_BASE_URL}/transactions/daily-stats?${params}`
    );
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
  ...
  getDailyReport: async (date: string): Promise<DailyReport> => {
    const response = await fetch(
      `${API_BASE_URL}/transactions/daily-report/${date}`
    );
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
  ...
  getMonthlyTrends: async (months?: number): Promise<MonthlyTrend[]> => {
    ...
    const response = await fetch(
      `${API_BASE_URL}/transactions/monthly-trends?${params}`
    );
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
