...
export function ExpenseChart({
  ...
  const renderChart = (
    ...
    const renderCustomLegend = () => {
      return (
        <ul style={{ listStyle: "none", padding: 0, marginLeft: 2 }}>
          {data.map((entry, index) => (
            ...
            >
              ...
              <span style={{ fontSize: 10 }}>
                {entry.category}: <strong>{formatAmount(entry.total)}</strong>
              </span>
            </li>
          ))}
        </ul>
    ...
    if (chartType === "pie") {
      return (
        <PieChart>
          ...
          <Tooltip
            formatter={(value: number) => [formatAmount(value), "Jumlah"]}
          />
          <Legend
        ...
        <CardHeader>
          ...
          <div className="flex gap-2">
            <Select
              ...
              onValueChange={setIncomeSelectedMonth}
            >
              <SelectTrigger className="w-48">
                <SelectValue />
              </SelectTrigger>
